...
### For contributors
- `validate.py` gained two checks for the failure classes above: a skill with a ```bash block must declare `Bash`, and a skill that resolves a skills root must probe `${CLAUDE_PLUGIN_ROOT}`. Both have self-test fixtures, and both were confirmed to fail against the real bugs before being committed.
